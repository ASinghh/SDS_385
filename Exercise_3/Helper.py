...
def line_search(B,alpha,c,x,y,p):
    while True:
        
        lg = logits(B,x)
        w = activation_sigmoid(lg)
        grad = (gradient(x,y,w))
        new_B = B - alpha*(grad)
        loss1 = neg_log_likely(y,w)
        
        loss2 = c*alpha*(LA.norm(grad)**2)
        new_lg = logits(new_B,x)
        new_w = activation_sigmoid(new_lg)
        loss3 = neg_log_likely(y,new_w)
        if loss3<= (loss1+loss2):
            break
        alpha = alpha*p
    return alpha
